...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const fee = Math.ceil(amount * 0.01); // 1% withdrawal fee
    ...
    // Deduct balance immediately (will be reversed if B2C fails)
    await supabase
      .from("wallets")
    ...
    // Call IntaSend B2C (Send Money) API
    ...
    // Update with IntaSend tracking info
    if (mpesaTx) {
      await supabase
        .from("mpesa_transactions")
